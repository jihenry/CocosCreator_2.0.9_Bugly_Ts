...
interface AppEventAPIs {
  onAudioInterruptionBegin: (options: () => void) => void;
  onAudioInterruptionEnd: (options: () => void) => void;
  onError: (options: (res: { message: string; stack: string }) => void) => void;
  offError: (options: () => void) => void;
  offAudioInterruptionEnd: (options: () => void) => void;
  offAudioInterruptionBegin: (options: () => void) => void;
}
 ...
 * 陀螺仪
 */
interface onGyroscopeChangeOpts {
  res: Object;
  x: number;
  y: number;
  z: number;
}
interface onGyroscopeChangeCallback {
  (res: onGyroscopeChangeOpts): void;
}
interface startGyroscopeOpts extends WxApiCallback {
  interval?: string;
}
...
interface DeviceAPIs {
   ...
   * 监听陀螺仪数据
   */
  onGyroscopeChange: (options: onGyroscopeChangeCallback) => void;
  /**
   * 开始监听陀螺仪数据
   */
  startGyroscope: (options?: startGyroscopeOpts) => void;
  /**
   * 停止监听陀螺仪数据
   */
  stopGyroscope: (options?: WxApiCallback) => void;
  /**
...
declare interface loadSubpackageOpts extends WxApiCallback {
  name: string;
}

declare interface LoadTaskRes {
  progress: number;
...
declare interface LoadSubpackageTask {
  onProgressUpdate: (cb: (data: LoadTaskRes) => void) => void;
}

interface subpackageAPIs {
  loadSubpackage: (options: loadSubpackageOpts) => LoadSubpackageTask;
}
...
interface OpenInterfaceAPIs {
  ...
  onHide: (func: () => void) => void;
  offShow: (func: () => void) => void;
  offHide: (func: () => void) => void;
  exitMiniProgram: (options: WxApiCallback) => void;
  addCard: any;
  openCard: any;
...
declare interface LogManager {
  debug: (...args: Object | Array.<any> | number | string) => void;
  info: (...args: Object | Array.<any> | number | string) => void;
  log: (...args: Object | Array.<any> | number | string) => void;
  warn(): (...args: Object | Array.<any> | number | string) => void;
}
interface ALDAPIs {
  aldSendEvent(eventName: string, paramMap?: string2AnyMap): void;
}

interface createFeedbackButtonOpts {
  type: string;
  text?: string;
  image?: string;
  style: {};
...
interface FeedbackAPIs {
  createFeedbackButton(options: createFeedbackButtonOpts): UserInfoButton;
}
